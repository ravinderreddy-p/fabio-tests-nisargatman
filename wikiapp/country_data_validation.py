...
def country_population_and_area_are_valid(continent_id, new_country_population, new_country_area):
    ...
    countries_count = Country.query.filter_by(continent_id=continent_id).count()
    if countries_count == 0:
        if continent_population >= new_country_population and continent_area >= new_country_area:
            return True
        else:
            app.logger.warning(f'Country area or population exceeds total '
                               f'Continent-ID: {continent_id} area or population')
            abort(400)

    existing_countries_population = Country.query.with_entities(func.sum(Country.population).label("sum")) \
        .filter_by(continent_id=continent_id).first()
